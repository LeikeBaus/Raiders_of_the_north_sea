...
class ActionPanel:
    ...
    def __init__(self, x: int, y: int, width: int, height: int):
        ...
        self.font_action = pygame.font.Font(None, ACTION_FONT_SIZE)
    ...
    def draw(self, screen: pygame.Surface, actions: List[Action], on_action_click: Callable[[Action], None]):
        ...
        # Title
        title = self.font_title.render(f"Legal Actions ({len(actions)})", True, BLACK)
        screen.blit(title, (self.x + 10, self.y + 10))
        
        # Instructions
        inst = self.font_tiny.render("Click action to execute | Scroll: mouse wheel", True, GRAY)
        screen.blit(inst, (self.x + 10, self.y + 40))
        
        if not actions:
            no_actions = self.font_action.render("No legal actions!", True, RED)
            screen.blit(no_actions, (self.x + 10, self.y + 70))
            return
        
        # Draw action list
        visible_actions = actions[self.scroll_offset:self.scroll_offset + ACTIONS_PER_PAGE]
        ...
        for i, action in enumerate(visible_actions):
            ...
            action_rect = pygame.Rect(
                self.x + 10,
                action_y + i * ACTION_ITEM_HEIGHT,
                self.width - 20,
                ACTION_ITEM_HEIGHT - 2
            )
            
            # Check hover
            is_hovered = action_rect.collidepoint(mouse_pos)
            color = GREEN if is_hovered else LIGHT_GRAY
            
            # Draw action button
            pygame.draw.rect(screen, color, action_rect)
            pygame.draw.rect(screen, BLACK, action_rect, 1)
            
            # Action text
            action_text = self._get_action_text(action)
            text_surface = self.font_action.render(action_text, True, BLACK)
        ...
        # Scroll indicator
        if len(actions) > ACTIONS_PER_PAGE:
            scroll_text = f"Showing {self.scroll_offset + 1}-{min(self.scroll_offset + ACTIONS_PER_PAGE, len(actions))} of {len(actions)}"
            scroll_surface = self.font_tiny.render(scroll_text, True, GRAY)
    ...
    def handle_scroll(self, direction: int, actions: List[Action]):
        ...
        if direction > 0:  # Scroll up
            ...
            max_scroll = max(0, len(actions) - ACTIONS_PER_PAGE)
